...
class Handler(BaseHTTPRequestHandler):
    ...
    def _do_GET_inner(self):
        ...
        if parsed.path == "/correlation.html":
            self._serve_static("correlation.html"); return
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  setRefresh(15);
    ...
    def _serve_static(self, filename):
        """Serviruje libovolny staticky HTML soubor z adresare serveru.
        Pouziva se pro correlation.html a dalsi prilohy."""
        # Bezpecnostni check - jen jednoduche jmeno bez cesty
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                html = f.read()
            # Auto-injection API URL pro correlation.html
            if filename == "correlation.html":
                inject = """<script>
window.addEventListener('DOMContentLoaded', () => {
  const apiUrl = window.location.origin;
  if (typeof API_URL !== 'undefined') {
    API_URL = apiUrl;
    localStorage.setItem('ceps_api_url', apiUrl);
    const modal = document.getElementById('apiModal');
    if (modal) modal.classList.remove('show');
    if (typeof loadAll === 'function') loadAll();
    if (typeof setRefresh === 'function') setRefresh(60);
  }
});
</script>"""
                html = html.replace("</body>", inject + "\n</body>")
