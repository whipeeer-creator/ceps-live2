    ...
    def _do_GET_inner(self):
        ...
        if parsed.path == "/health":
            self._json({"status": "ok", "time": datetime.now().isoformat(),
                        "version": "regelleistung-xlsx-v25-threaded"}); return
    ...
    def _wind_de(self, qs):
        ...
        try:
            ...
            if not results:
                ...
                if cache["data"]:
                    print(f"  -> Wind-DE: all 3 failed, returning stale cache "
                          f"(age={int(now - cache['ts'])}s)", flush=True)
                    out = dict(cache["data"])
                    out["_cache"] = "stale"
                    out["_age_sec"] = int(now - cache["ts"])
...
if __name__ == "__main__":
    ...
    print(f"VERSION: regelleistung-xlsx-v25-threaded", flush=True)
