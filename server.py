...
class Handler(BaseHTTPRequestHandler):
    ...
    def _do_GET_inner(self):
        ...
        if parsed.path == "/health":
            self._json({"status": "ok", "time": datetime.now().isoformat(),
                        "version": "regelleistung-xlsx-v29-hourly-spread"}); return
    ...
    def _ote_last7d_stats(self, qs):
        ...
        try:
            ...
            # Seskup ceny po dnech (Berlin TZ) - agregovane na HODINY
            # Energy-Charts vraci 15min data, my potrebujeme hodinove pro realisticky DA spread
            from collections import defaultdict
            hourly_buckets = defaultdict(list)  # (day, hour) -> list of 15min prices
            for ts, price in zip(timestamps, prices):
                if price is None:
                    continue
                dt_utc = datetime.fromtimestamp(ts, tz=timezone.utc)
                ...
                hour = dt_berlin.hour
                hourly_buckets[(day_key, hour)].append(float(price))

            # Agreguj 15min -> hodinove ceny (prumer)
            days = defaultdict(list)
            for (day_key, hour), qh_prices in hourly_buckets.items():
                if qh_prices:
                    hourly_avg = sum(qh_prices) / len(qh_prices)
                    days[day_key].append(hourly_avg)
    ...
    def _weather(self, qs):
        ...
        Cache 60 minut + stale-while-revalidate pri rate limitu nebo erroru.
        """
        try:
            ...
            now = time.time()
            # Cerstvy cache - 3 hodiny (Open-Meteo rate limit safety)
            ...
            try:
                ...
                fetch_failed = (r.status_code != 200)
                fetch_error = f"HTTP {r.status_code}" if fetch_failed else None
            except Exception as fetch_e:
                fetch_failed = True
                fetch_error = str(fetch_e)
                r = None

            # Pri selhani - vrat stale cache pokud existuje (i stary nez nic)
            if fetch_failed:
                if cache["data"]:
                    print(f"  -> Weather: API failed ({fetch_error}), returning stale cache "
                          f"(age={int(now - cache['ts'])}s)", flush=True)
                    out = dict(cache["data"])
                    out["_cache"] = "stale"
                    out["_age_sec"] = int(now - cache["ts"])
                    out["_warning"] = f"Open-Meteo {fetch_error}, vraceny stale data"
                    self._json(out); return
                # Zadna cache - vrat error
                self._json({"error": f"Open-Meteo {fetch_error}"}, 502); return
    ...
    def _wind_de(self, qs):
        ...
        Hamburg, Bremerhaven, Husum. Open-Meteo API.
        Cache 60 minut + stale-while-revalidate.
        """
        try:
            ...
            # Open-Meteo umi multiple latitudes/longitudes v jednom requestu (oddelene carkou)
            # = 1 request misto 3 = 3x mensi spotreba rate limitu
            ...
            try:
                ...
                if r.status_code == 200:
                    data = r.json()
                    # Multi-location vraci pole objektu, jeden per lokace
                    if isinstance(data, list):
                        results = data
                    else:
                        results = [data]  # Single location = single object (fallback)
                else:
                    print(f"  -> wind-de HTTP {r.status_code}", flush=True)
            except Exception as e:
                print(f"  -> wind-de fail: {e}", flush=True)

            if not results:
                # Vsechny 3 mesta failnula - vrat stale cache pokud existuje
                if cache["data"]:
                    print(f"  -> Wind-DE: failed, returning stale cache "
                          f"(age={int(now - cache['ts'])}s)", flush=True)
                    out = dict(cache["data"])
                    out["_cache"] = "stale"
                    out["_age_sec"] = int(now - cache["ts"])
                    out["_warning"] = "Open-Meteo selhalo, vraceny stale data"
                    self._json(out); return
                self._json({"error": "Open-Meteo failed"}, 502); return
...
if __name__ == "__main__":
    ...
    print(f"VERSION: regelleistung-xlsx-v29-hourly-spread", flush=True)
