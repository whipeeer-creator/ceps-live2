...
class Handler(BaseHTTPRequestHandler):
    ...
    def _entsoe_residual_load(self, qs):
        ...
        try:
            ...
            # Berlin TZ - dnesek + zitra
            now_utc = datetime.now(timezone.utc)
            month = now_utc.month
            berlin_offset = 2 if 4 <= month <= 10 else 1
            berlin_now = now_utc + timedelta(hours=berlin_offset)
            ps_local = berlin_now.replace(hour=0, minute=0, second=0, microsecond=0)
            pe_local = ps_local + timedelta(hours=48)
    ...
    def _ote_vdt(self, qs):
        ...
        try:
            ...
            for row in rows_iter:
                ...
                # Ceny zacinaji za sloupcem s intervalem
                # Sloupce: [interval] [vazeny_avg] [min] [max] [last]
                offset = interval_col + 1
                weighted_avg = parse_price(row[offset] if len(row) > offset else "")
                min_price = parse_price(row[offset + 1] if len(row) > offset + 1 else "")
                max_price = parse_price(row[offset + 2] if len(row) > offset + 2 else "")
                last_price = parse_price(row[offset + 3] if len(row) > offset + 3 else "")
