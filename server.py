    ...
    def _ote_vdt(self, qs):
        ...
        try:
            ...
            for row in rows_iter:
                ...
                interval = (row[0] or "").strip()

                # Detekce data row: format "HH:MM-HH:MM"
                if "-" not in interval or ":" not in interval:
                    continue

                try:
                    start_str, _ = interval.split("-")
                    start_h, start_m = start_str.strip().split(":")
                    start_h = int(start_h)
                    start_m = int(start_m)
                    if not (0 <= start_h <= 24 and start_m in (0, 15, 30, 45)):
                        continue
                except (ValueError, IndexError):
                    continue

                weighted_avg = parse_price(row[1] if len(row) > 1 else "")
                min_price = parse_price(row[2] if len(row) > 2 else "")
                max_price = parse_price(row[3] if len(row) > 3 else "")
                last_price = parse_price(row[4] if len(row) > 4 else "")
