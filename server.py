...
class Handler(BaseHTTPRequestHandler):
    ...
    def _ote_vdt(self, qs):
        ...
        try:
            ...
            # Parse XLSX - hledame "Posledni cena" sloupec
            points = []
            header_row_idx = None
            last_col = None
            interval_col = None
            ...
            for row_idx, cells in enumerate(_xlsx_iter_rows(xlsx_bytes)):
                # Filter na 2-tuple (col, val) - obcas vraci prazdne
                cells = [c for c in cells if isinstance(c, (tuple, list)) and len(c) == 2]
                ...
                if header_row_idx is None:
                    ...
                    if header_row_idx is not None:
                        for col, val in cells:
                            if isinstance(val, str):
                                vlow = val.lower()
                                if "interval" in vlow or "čas" in vlow:
                                    interval_col = col
                                    break
                    continue
                
                if last_col and last_col in cell_dict:
                    interval = cell_dict.get(interval_col) if interval_col else (
                        cell_dict.get("A") or cell_dict.get("B") or cell_dict.get("C"))
                    last_val = cell_dict.get(last_col)
                    
                    if isinstance(interval, str) and ":" in interval and last_val is not None:
                        try:
                            time_start = interval.split("-")[0].strip()
                            parts = time_start.split(":")
                            if len(parts) < 2: continue
                            hh, mm = parts[0], parts[1]
                            ts = day_dt.replace(hour=int(hh), minute=int(mm))
            ...
            if not points and all_rows:
                # DEBUG: vypsat strukturu prvnich 10 radku
                print(f"  -> VDT {date_str}: header parsing FAILED, debug first 10 rows:", flush=True)
                for i, cd in enumerate(all_rows[:10]):
                    print(f"     row {i}: {dict(list(cd.items())[:15])}", flush=True)
                
                for cell_dict in all_rows:
                    # Najdi cas (HH:MM-HH:MM nebo jen HH:MM)
                    interval = None
                    ...
                    try:
                        ...
                        # Nejpravejsi numericky sloupec = posledni cena typicky
