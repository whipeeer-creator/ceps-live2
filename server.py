    ...
    def do_GET(self):
        ...
        if parsed.path == "/health":
            self._json({"status": "ok", "time": datetime.now().isoformat(),
                        "version": "regelleistung-xlsx-v17-history"}); return
        ...
        try:
            ...
            try:
                ...
                if r2.status_code == 200:
                    ...
                    # Fallback pro vcerejsek - stahnout ze spot.56k.guru jen kdyz history je prazdna
                    if not hours_yesterday and current_hour is not None and current_hour < 8:
                        try:
                            ...
                            if r3.status_code == 200:
                                hass = r3.json()
                                # Vyzkoumat strukturu - muze byt {data: [...]} nebo {attributes: {data: [...]}}
                                all_data = (hass.get("data") or
                                           hass.get("attributes", {}).get("data") or
                                           hass.get("today", []) + hass.get("yesterday", []))
                                hours_yesterday = []
                                for entry in all_data:
                                    ts_str = entry.get("st") or entry.get("start") or entry.get("startsAt") or ""
                                    if ts_str.startswith(yesterday_date_str):
                                        try:
                                            ts_part = ts_str.split("T")[1][:2]
                                            hr = int(ts_part)
                                            price_eur = entry.get("p") or entry.get("price") or entry.get("value")
                                            if price_eur is not None:
                                                hours_yesterday.append({
                                                    "hour": hr,
                                                    "priceEur": float(price_eur),
                                                    "priceCZK": int(float(price_eur) * 24.35),
                                                })
                                        except (ValueError, IndexError):
                                            continue
                                if hours_yesterday:
                                    print(f"  -> spot.56k.guru: {len(hours_yesterday)}h vcerejsich dat", flush=True)
                                    history[yesterday_date_str] = hours_yesterday  # taky uloz do historie
                        except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"VERSION: regelleistung-xlsx-v17-history", flush=True)
