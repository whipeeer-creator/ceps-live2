...
class Handler(BaseHTTPRequestHandler):
    ...
    def _do_GET_inner(self):
        ...
        if parsed.path == "/health":
            self._json({"status": "ok", "time": datetime.now().isoformat(),
                        "version": "regelleistung-xlsx-v32-cena-re"}); return
    ...
    def _entsoe_residual_load(self, qs):
        ...
        Cache 1h. Vraci hodinove forecasty na dnesek + zitra (48h dopredu).
        """
        try:
            if "_RESIDUAL_LOAD_CACHE" not in globals():
                globals()["_RESIDUAL_LOAD_CACHE"] = {"ts": 0, "data": None}
            cache = globals()["_RESIDUAL_LOAD_CACHE"]
            now_ts = time.time()
            if cache["data"] and (now_ts - cache["ts"]) < 900:  # 15 min cache
                out = dict(cache["data"]); out["_cache"] = "hit"
                out["_age_sec"] = int(now_ts - cache["ts"])
                self._json(out); return

            # Berlin TZ - poslednich 30 dni + zitra
            now_utc = datetime.now(timezone.utc)
            month = now_utc.month
            berlin_offset = 2 if 4 <= month <= 10 else 1
            berlin_now = now_utc + timedelta(hours=berlin_offset)
            ps_local = berlin_now.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=30)
            ...
            # 1b) Demand ACTUAL (A65 + A16 Realised total load)
            demand_act_xml, demand_act_st = call_entsoe({
                "documentType": "A65",
                "processType":  "A16",
                "outBiddingZone_Domain": DE_LU_DOMAIN,
                "periodStart":  period_start,
            ...
            demand_act_pts = parse_entsoe_xml(demand_act_xml, ps) if demand_act_st == 200 else []
            print(f"  -> ENTSO-E Demand ACTUAL DE: status={demand_act_st}, points={len(demand_act_pts)}", flush=True)
            ...
            # 2b) Wind onshore ACTUAL (A75 + A16 Realised)
            wind_on_act_xml, wind_on_act_st = call_entsoe({
                "documentType": "A75",
                "processType":  "A16",
                "psrType":      "B19",
                "in_Domain":    DE_LU_DOMAIN,
            ...
            wind_on_act_pts = parse_entsoe_xml(wind_on_act_xml, ps) if wind_on_act_st == 200 else []
            print(f"  -> ENTSO-E Wind onshore ACTUAL DE: status={wind_on_act_st}, points={len(wind_on_act_pts)}", flush=True)

            # 2c) Wind offshore ACTUAL
            wind_off_act_xml, wind_off_act_st = call_entsoe({
                "documentType": "A75",
                "processType":  "A16",
                "psrType":      "B18",
                "in_Domain":    DE_LU_DOMAIN,
            ...
            wind_off_act_pts = parse_entsoe_xml(wind_off_act_xml, ps) if wind_off_act_st == 200 else []
            print(f"  -> ENTSO-E Wind offshore ACTUAL DE: status={wind_off_act_st}, points={len(wind_off_act_pts)}", flush=True)

            # 2d) Wind onshore INTRADAY forecast (A69 + A40 Intraday)
            wind_on_id_xml, wind_on_id_st = call_entsoe({
                "documentType": "A69",
                "processType":  "A40",
                "psrType":      "B19",
                "in_Domain":    DE_LU_DOMAIN,
            ...
            wind_on_id_pts = parse_entsoe_xml(wind_on_id_xml, ps) if wind_on_id_st == 200 else []
            print(f"  -> ENTSO-E Wind onshore ID DE: status={wind_on_id_st}, points={len(wind_on_id_pts)}", flush=True)

            # 2e) Wind offshore INTRADAY forecast
            wind_off_id_xml, wind_off_id_st = call_entsoe({
                "documentType": "A69",
                "processType":  "A40",
                "psrType":      "B18",
                "in_Domain":    DE_LU_DOMAIN,
            ...
            wind_off_id_pts = parse_entsoe_xml(wind_off_id_xml, ps) if wind_off_id_st == 200 else []
            print(f"  -> ENTSO-E Wind offshore ID DE: status={wind_off_id_st}, points={len(wind_off_id_pts)}", flush=True)
            ...
            # 4b) Solar ACTUAL (A75 + A16 Realised, B16)
            solar_act_xml, solar_act_st = call_entsoe({
                "documentType": "A75",
                "processType":  "A16",
                "psrType":      "B16",
                "in_Domain":    DE_LU_DOMAIN,
            ...
            solar_act_pts = parse_entsoe_xml(solar_act_xml, ps) if solar_act_st == 200 else []
            print(f"  -> ENTSO-E Solar ACTUAL DE: status={solar_act_st}, points={len(solar_act_pts)}", flush=True)
            ...
            demand_act_d = to_dict(demand_act_pts)
            wind_on_d = to_dict(wind_on_pts)
            wind_off_d = to_dict(wind_off_pts)
            wind_on_act_d = to_dict(wind_on_act_pts)
            wind_off_act_d = to_dict(wind_off_act_pts)
            wind_on_id_d = to_dict(wind_on_id_pts)
            wind_off_id_d = to_dict(wind_off_id_pts)
            solar_d = to_dict(solar_pts)
            solar_act_d = to_dict(solar_act_pts)

            # Vsechny ts klicove napric vsemi datasety
            all_ts = sorted(set(demand_d.keys()) | set(wind_on_d.keys())
                          | set(wind_off_d.keys()) | set(solar_d.keys())
                          | set(wind_on_act_d.keys()) | set(wind_off_act_d.keys())
                          | set(wind_on_id_d.keys()) | set(wind_off_id_d.keys())
                          | set(demand_act_d.keys()) | set(solar_act_d.keys()))

            result = []
            for ts in all_ts:
                d = demand_d.get(ts)
                d_act = demand_act_d.get(ts)
                wo = wind_on_d.get(ts, 0)
                woff = wind_off_d.get(ts, 0)
                s = solar_d.get(ts, 0)
                s_act = solar_act_d.get(ts)
                wind_total = (wo or 0) + (woff or 0)
                # Wind actual
                wo_act = wind_on_act_d.get(ts)
                woff_act = wind_off_act_d.get(ts)
                wind_actual = None
                if wo_act is not None or woff_act is not None:
                    wind_actual = (wo_act or 0) + (woff_act or 0)
                # Wind ID forecast
                wo_id = wind_on_id_d.get(ts)
                woff_id = wind_off_id_d.get(ts)
                wind_id = None
                if wo_id is not None or woff_id is not None:
                    wind_id = (wo_id or 0) + (woff_id or 0)
                
                # Residual ACTUAL = demand_actual - wind_actual - solar_actual
                residual_actual = None
                if d_act is not None and wind_actual is not None and s_act is not None:
                    residual_actual = d_act - wind_actual - s_act

                if d is None:
                    # Pokud chybí demand forecast, ale máme actual, použij ho
                    if d_act is None:
                        continue
                    d = d_act
                ...
                result.append({
                    ...
                    "demand_mw": round(d, 1),
                    "demand_actual_mw": round(d_act, 1) if d_act is not None else None,
                    "wind_mw": round(wind_total, 1),
                    "wind_actual_mw": round(wind_actual, 1) if wind_actual is not None else None,
                    "wind_id_mw": round(wind_id, 1) if wind_id is not None else None,
                    "solar_mw": round(s or 0, 1),
                    "solar_actual_mw": round(s_act, 1) if s_act is not None else None,
                    "residual_load_mw": round(residual, 1),
                    "residual_load_actual_mw": round(residual_actual, 1) if residual_actual is not None else None,
                })
    ...
    def _smard_residual_load(self, qs):
        """SMARD (Bundesnetzagentur) - residuallast pro DE.
        Rychlejsi nez ENTSO-E (~30min delay misto 60-90min).
        Filter 4359 = Residuallast, region DE, quarterhour resolution.
        """
        try:
            # Cache 5 minut (SMARD je rychlejsi nez ENTSO-E)
            if "_SMARD_CACHE" not in globals():
                globals()["_SMARD_CACHE"] = {"ts": 0, "data": None}
            cache = globals()["_SMARD_CACHE"]
            now_ts = time.time()
            if cache["data"] and (now_ts - cache["ts"]) < 300:
                out = dict(cache["data"]); out["_cache"] = "hit"
                out["_age_sec"] = int(now_ts - cache["ts"])
                self._json(out); return

            # 1) Najdi nejnovejsi timestamp (SMARD pouziva svuj vlastni)
            # GET https://www.smard.de/app/chart_data/4359/DE/index_quarterhour.json
            idx_url = "https://www.smard.de/app/chart_data/4359/DE/index_quarterhour.json"
            headers = {"User-Agent": "Mozilla/5.0 (compatible; ceps-live2)"}
            ...
            if not timestamps:
                self._json({"error": "no timestamps from SMARD"}, 502); return
            
            # SMARD vraci timestamps weekly (zacatek tydne v ms)
            # Stahnout poslednich nekolik tydnu pro 30 dni dat
            timestamps.sort()
            # 30 dni / 7 = ~5 tydnu, vezmeme 6 pro jistotu
            recent_weeks = timestamps[-6:]
            
            all_series = []
            for ts in recent_weeks:
                try:
                    data_url = f"https://www.smard.de/app/chart_data/4359/DE/4359_DE_quarterhour_{ts}.json"
                    r2 = requests.get(data_url, headers=headers, timeout=15)
                    r2.raise_for_status()
                    wk_data = r2.json()
                    series = wk_data.get("series", [])
                    all_series.extend(series)
                except Exception as e:
                    print(f"  -> SMARD week {ts} ERROR: {e}", flush=True)
                    continue
            
            # SMARD format: [[unix_ms, value_mw_or_null], ...]
            # Transformovat na nase format
            data_out = []
            for entry in all_series:
                ...
                if val is None:
                    continue
                # Berlin TZ
                dt_utc = datetime.fromtimestamp(ms_ts / 1000, tz=timezone.utc)
                month = dt_utc.month
                berlin_offset = 2 if 4 <= month <= 10 else 1
                dt_berlin = dt_utc + timedelta(hours=berlin_offset)
                data_out.append({
                    "ts": dt_utc.strftime("%Y-%m-%dT%H:%M") + "Z",
                    "berlin_time": dt_berlin.strftime("%Y-%m-%dT%H:%M"),
                    "residual_mw": round(float(val), 1),
                })
            
            # Razeni a deduplikace dle ts
            seen = set()
            unique = []
            for d in sorted(data_out, key=lambda x: x["ts"]):
                if d["ts"] not in seen:
                    seen.add(d["ts"])
                    unique.append(d)
    ...
    def _ote_vdt(self, qs):
        """Vraci VDT (vnitrodenni trh) ceny z OTE-CR XLSX.

        Stahuje:
          https://www.ote-cr.cz/pubweb/attachments/27/YYYY/monthMM/dayDD/
          VDT_15MIN_DD_MM_YYYY_CZ.xlsx

        Struktura XLSX (4 cenove sloupce + interval):
          A: Casovy interval (napr. "11:30-11:45")
          B: Vazeny prumer cen [EUR/MWh]
          C: Minimalni cena [EUR/MWh]
          D: Maximalni cena [EUR/MWh]
          E: Posledni cena [EUR/MWh]    <- klicove pro nas

        Granularita: 15-min QH (96 radku za den).
        Zpozdeni: ~1h po skonceni QH.
        Cache: 5 minut pro dnesek, 1h pro starsi dny.
        """
        import urllib.request
        from datetime import datetime as _dt

        # Datum: defaultne dnes, format YYYY-MM-DD
        date_str = qs.get("date", [""])[0]
        if date_str:
            try:
                target_date = _dt.strptime(date_str, "%Y-%m-%d")
            except ValueError:
                self._json({"error": "Bad date format, use YYYY-MM-DD"}, 400)
                return
        else:
            target_date = _dt.now()
        ...
        # Globalni cache na urovni modulu
        global _VDT_CACHE
        try:
            _VDT_CACHE
        except NameError:
        ...
        if cache_key in _VDT_CACHE:
            cached_time, cached_data = _VDT_CACHE[cache_key]
            is_today = target_date.date() == now.date()
            max_age = 300 if is_today else 3600
            if (now - cached_time).total_seconds() < max_age:
                self._json(cached_data)
                return

        # URL podle OTE patternu
        yyyy = target_date.strftime("%Y")
        mm = target_date.strftime("%m")
        dd = target_date.strftime("%d")
        file_name = f"VDT_15MIN_{dd}_{mm}_{yyyy}_CZ.xlsx"
        url = (
            f"https://www.ote-cr.cz/pubweb/attachments/27/{yyyy}/"
            f"month{mm}/day{dd}/{file_name}"
        )

        print(f"  -> /ote/vdt fetching {url}", flush=True)

        try:
            req = urllib.request.Request(
                url,
                headers={"User-Agent": "Mozilla/5.0 (CEPS-Live VDT fetcher)"}
            )
            with urllib.request.urlopen(req, timeout=20) as r:
                xlsx_bytes = r.read()

            print(f"  -> /ote/vdt downloaded {len(xlsx_bytes)} bytes", flush=True)

            # Parse XLSX pomoci existujiciho streameru
            rows_iter = _xlsx_iter_rows(xlsx_bytes)

            data_points = []
            rows_seen = 0
            debug_first_rows = []  # Prvnich 10 radku pro debug

            def parse_price(s):
                """Parsuje cenu (cestina pouziva carku jako desetinnou tecku)."""
                if s is None:
                    return None
            ...
            def parse_interval(s):
                """Parsuje interval 'HH:MM-HH:MM' nebo 'HH:MM - HH:MM'.
                Vraci (start_h, start_m) nebo None."""
                if not s:
                    return None
                s = str(s).strip().replace(" ", "").replace("\xa0", "")
                # Hledame dvojtecku a pomlcku
                if ":" not in s:
                    return None
                # Formaty: "11:30-11:45", "11:30—11:45", atd.
                for sep in ("-", "—", "–", "to"):
                    if sep in s:
                        parts = s.split(sep, 1)
                        if len(parts) == 2:
            ...
            for row in rows_iter:
                ...
                if rows_seen <= 10:
                    debug_first_rows.append(row[:8] if row else [])

                if not row or len(row) < 2:
                    continue

                # Zkus najit interval v ruznych sloupcich (A, B nebo C)
                interval_info = None
                interval_col = -1
                for try_col in range(min(3, len(row))):
                    info = parse_interval(row[try_col])
                    if info is not None:
                        interval_info = info
                        interval_col = try_col
                        break

                if interval_info is None:
                    continue

                start_h, start_m = interval_info

                # Struktura OTE VDT XLSX:
                #   A: Casovy interval
                #   B: Zobchodovane mnozstvi
                #   C: Zobchodovane mnozstvi - nakup
                #   D: Zobchodovane mnozstvi - prodej
                #   E: Vazeny prumer cen (EUR/MWh)
                #   F: Minimalni cena
                #   G: Maximalni cena
                #   H: Posledni cena
                # Cenové sloupce začínají 4 sloupce za intervalem (offset+3)
                offset = interval_col + 1
                # offset+0..2 = množství, offset+3..6 = ceny
                weighted_avg = parse_price(row[offset + 3] if len(row) > offset + 3 else "")
                min_price    = parse_price(row[offset + 4] if len(row) > offset + 4 else "")
                max_price    = parse_price(row[offset + 5] if len(row) > offset + 5 else "")
                last_price   = parse_price(row[offset + 6] if len(row) > offset + 6 else "")

                # ISO timestamp
                ts = target_date.replace(
                    hour=start_h if start_h < 24 else 0,
                    minute=start_m, second=0, microsecond=0
                )
                if start_h >= 24:
                ...
                has_data = any(v is not None for v in
                              (weighted_avg, last_price, min_price, max_price))

                interval_str = (row[interval_col] or "").strip() if interval_col < len(row) else ""
            ...
            print(f"  -> /ote/vdt parsed {len(data_points)} points "
                  f"(of {rows_seen} rows)", flush=True)
            if len(data_points) == 0 and debug_first_rows:
                print(f"  -> /ote/vdt DEBUG first rows:", flush=True)
                for i, r in enumerate(debug_first_rows):
                    print(f"     row[{i}]: {r!r}", flush=True)

            result = {
                "date": target_date.strftime("%Y-%m-%d"),
                "data": data_points,
                "source": "ote-cr.cz VDT 15-min XLSX",
                "url": url,
                "fetched_at": now.strftime("%Y-%m-%dT%H:%M:%S"),
            }

            _VDT_CACHE[cache_key] = (now, result)

            self._json(result)

        except urllib.error.HTTPError as e:
            # Pro budouci dny / vikendy XLSX nemusi existovat
            if e.code == 404:
                self._json({
                    "date": target_date.strftime("%Y-%m-%d"),
                    "data": [],
                    "error": "VDT data not yet published for this date",
                    "url": url,
                }, 200)
                return
            print(f"  -> /ote/vdt HTTP ERROR {e.code}: {e}", flush=True)
            self._json({"error": f"HTTP {e.code}", "url": url}, 502)
        except Exception as e:
            print(f"  -> /ote/vdt ERROR: {e}", flush=True)
            import traceback
            traceback.print_exc()
            self._json({"error": str(e), "url": url}, 502)

    def _ote_vdt_range(self, qs):
        """Vraci VDT data za rozsah dni (1-31). 
        
        Query params:
          days_back: kolik dni zpet (default 7, max 31)
        
        Pouziva interni cache _VDT_CACHE pro kazdy den separatne.
        Pro stare dny (vcera a starsi) se data nikdy nemeni - cache navzdy.
        Pro dnesek cache 5 min.
        Stahovani paralelni (ThreadPoolExecutor) - 30 dni za ~3-5s misto ~30s.
        """
        ...
        try:
            ...
            days_back = max(1, min(31, days_back))
        except (ValueError, IndexError):
            days_back = 7
        ...
        def fetch_day(target_date):
            """Stahne data pro 1 den. Vraci (cache_key, day_data, success, was_cached)."""
            cache_key = target_date.strftime("%Y-%m-%d")
            is_today = target_date.date() == now.date()
            ...
            if cache_key in _VDT_CACHE:
                cached_time, cached_data = _VDT_CACHE[cache_key]
                max_age = 300 if is_today else 86400
                if (now - cached_time).total_seconds() < max_age:
                    return (cache_key, cached_data, True, True)
            ...
            try:
                req = urllib.request.Request(
                    url, headers={"User-Agent": "Mozilla/5.0 (CEPS-Live VDT range)"}
                )
                # Retry logic - když selže, zkus ještě jednou (OTE někdy rate-limituje)
                xlsx_bytes = None
                last_err = None
                for attempt in range(2):
                    try:
                        ...
                        last_err = e
                        if attempt == 0:
                            time.sleep(0.5)  # krátká pauza před retry
                            continue
                        raise
                if xlsx_bytes is None:
                    raise last_err or Exception("no data")
                ...
                for row in rows_iter:
                    ...
                    for try_col in range(min(3, len(row))):
                        ...
                        if info is not None:
                            ...
                            interval_col = try_col
                            break
                    if interval_info is None: continue
                    ...
                    has_data = any(v is not None for v in (weighted_avg, last_price, min_price, max_price))
                    interval_str = (row[interval_col] or "").strip() if interval_col < len(row) else ""
                ...
                cached = {"date": cache_key, "data": day_data, "url": url}
                _VDT_CACHE[cache_key] = (now, cached)
                return (cache_key, cached, True, False)
            
            except urllib.error.HTTPError as e:
                if e.code == 404:
                    cached = {"date": cache_key, "data": [], "error": "404"}
                    _VDT_CACHE[cache_key] = (now, cached)
                print(f"  -> /ote/vdt/range HTTP {e.code} pro {cache_key}: {e}", flush=True)
                return (cache_key, None, False, False)
            except Exception as e:
                import traceback
                print(f"  -> /ote/vdt/range error pro {cache_key}: {type(e).__name__}: {e}", flush=True)
                traceback.print_exc()
                return (cache_key, None, False, False)
        
        # Pripravit seznam dnu
        target_dates = [now - _td(days=i) for i in range(days_back, -1, -1)]
        
        # Paralelni stahovani (max 4 concurrent connections aby OTE nenarazel)
        results_by_key = {}
        try:
            with ThreadPoolExecutor(max_workers=4) as executor:
                futures = {executor.submit(fetch_day, d): d for d in target_dates}
                for future in as_completed(futures, timeout=60):
                    try:
                        cache_key, cached, success, was_cached = future.result(timeout=10)
                        results_by_key[cache_key] = cached
                        if success and not was_cached:
                            days_loaded += 1
                        elif not success:
                            days_failed += 1
                    except Exception as e:
                        print(f"  -> /ote/vdt/range future error: {e}", flush=True)
                        days_failed += 1
        except Exception as e:
            print(f"  -> /ote/vdt/range pool error: {e}", flush=True)
        
        # Sestavit data v chronologickem poradi
        all_data = []
        for d in target_dates:
            key = d.strftime("%Y-%m-%d")
            cached = results_by_key.get(key)
            if cached and "data" in cached:
                all_data.extend(cached["data"])
        
        print(f"  -> /ote/vdt/range: {days_back}+1 dnu, "
              f"loaded={days_loaded}, failed={days_failed}, "
              f"total points={len(all_data)}", flush=True)
        
        result = {
            "days_back": days_back,
            "data": all_data,
            "loaded_days": days_loaded,
            "failed_days": days_failed,
            "fetched_at": now.strftime("%Y-%m-%dT%H:%M:%S"),
        }
        self._json(result)
  ...
  setRefresh(15);
    ...
    def _html_hruska(self):
        ...
        try:
            with open(html_path, "r", encoding="utf-8") as f:
                html = f.read()
            # Auto-set API URL na origin server URL (bez setup modalu)
            inject = """<script>
window.addEventListener('DOMContentLoaded', () => {
