...
class Handler(BaseHTTPRequestHandler):
    ...
    def _entsoe_cross_border(self, qs):
        """Physical Flows pro CZ borders. Vraci CZ↔DE, CZ↔AT, CZ↔SK, CZ↔PL.
        ...
        try:
            ...
            now_dt = datetime.utcnow()
            ps = now_dt - timedelta(hours=24)
            pe = now_dt + timedelta(hours=2)
            period_start = fmt_entsoe_period(ps)
            period_end = fmt_entsoe_period(pe)
            
            def fetch_flow(name, in_domain, out_domain):
                """Physical flow pro jeden smer."""
                try:
                    ...
                    return (name, [{"ts": p["ts"], "mw": p["value"]} for p in pts])
                except Exception as e:
                    print(f"  -> cross-border {name}: {e}", flush=True)
                    return (name, [])
            
            borders = [
                ("CZ_DE_in",  CZ, DE),  # DE → CZ (import do CZ)
                ("CZ_DE_out", DE, CZ),  # CZ → DE (export z CZ)
                ("CZ_AT_in",  CZ, AT),
                ("CZ_AT_out", AT, CZ),
                ("CZ_SK_in",  CZ, SK),
                ("CZ_SK_out", SK, CZ),
            ...
            with ThreadPoolExecutor(max_workers=8) as executor:
                futures = [executor.submit(fetch_flow, n, i, o) for n, i, o in borders]
                for f in futures:
                    name, points = f.result(timeout=20)
                    results[name] = points
            
            def net_flow(in_key, out_key):
                ...
                return [{
                    "ts": ts,
                    "in_mw": in_map.get(ts, 0),
                    "out_mw": out_map.get(ts, 0),
                    "net_mw": in_map.get(ts, 0) - out_map.get(ts, 0),
                } for ts in all_ts]
            
            out = {
                "borders": {
                    "CZ_DE": net_flow("CZ_DE_in", "CZ_DE_out"),
                    "CZ_AT": net_flow("CZ_AT_in", "CZ_AT_out"),
                    "CZ_SK": net_flow("CZ_SK_in", "CZ_SK_out"),
                    "CZ_PL": net_flow("CZ_PL_in", "CZ_PL_out"),
                },
                "country": "CZ",
                "source": "ENTSO-E (A11 Physical Flow)",
                "fetched_at": now_dt.strftime("%Y-%m-%dT%H:%M:%SZ"),
                "_cache": "miss",
            }
            self._CROSSBORDER_CACHE = {"ts": now_ts, "data": out}
            print(f"  -> /entsoe/cross-border: 4 borders fetched", flush=True)
            self._json(out)
        except Exception as e:
