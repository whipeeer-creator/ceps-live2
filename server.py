    ...
    def do_GET(self):
        ...
        if parsed.path == "/health":
            self._json({"status": "ok", "time": datetime.now().isoformat(),
                        "version": "regelleistung-xlsx-v16-yesterday"}); return
        ...
        try:
            ...
            try:
                ...
                if r2.status_code == 200:
                    ...
                    hours_yesterday = day_data.get("hoursYesterday", [])  # spotovaelektrina.cz toto NEVRACI

                    # Fallback pro vcerejsek - stahnout ze spot.56k.guru (ENTSO-E proxy, free)
                    # Pouzijeme jen kdyz potrebujeme (current_hour < 7)
                    if not hours_yesterday and current_hour is not None and current_hour < 7:
                        try:
                            r3 = _request_with_retry(
                                requests.get,
                                "https://spot.56k.guru/api/v2/hass",
                                params={
                                    "currency": "EUR", "area": "CZ",
                                    "extra": "0", "multiplier": "1",
                                    "factor": "1", "decimals": "2",
                                    "interval": "PT60M"
                                },
                                timeout=15,
                            ...
                            if r3.status_code == 200:
                                hass = r3.json()
                                # data: pole vsech dostupnych hodin (yesterday + today + tomorrow)
                                # Format: {"data": [{"st": "2026-05-06T00:00:00+02:00", "p": 145.32}, ...]}
                                all_data = hass.get("data") or hass.get("attributes", {}).get("data") or []
                                # Najdi vcerejsi hodiny
                                now_utc = datetime.now(timezone.utc)
                                month = now_utc.month
                                berlin_offset = 2 if 4 <= month <= 10 else 1
                                berlin_now = now_utc + timedelta(hours=berlin_offset)
                                ...
                                for entry in all_data:
                                    ts_str = entry.get("st") or entry.get("start") or ""
                                    if ts_str.startswith(yesterday_date):
                                        try:
                                            # Vytahnout hodinu z timestampu
                                            ts_part = ts_str.split("T")[1][:2]
                                            hr = int(ts_part)
                                            price_eur = entry.get("p") or entry.get("price")
                                            if price_eur is not None:
                                                hours_yesterday.append({
                                                    "hour": hr,
                                                    "priceEur": float(price_eur),
                                                    "priceCZK": int(float(price_eur) * 24.35),  # priblizny kurz
                                                })
                                        except (ValueError, IndexError):
                                            continue
                                if hours_yesterday:
                                    print(f"  -> spot.56k.guru: stazeno {len(hours_yesterday)}h vcerejsich dat", flush=True)
                        except Exception as e:
                            print(f"  -> spot.56k.guru fallback failed: {e}", flush=True)
                    ...
                    if current_hour is not None and hours_today:
                        ...
                        for offset in range(-7, 1):
                            ...
                            if target_hour >= 0:
                                ...
                                # Predchozi den - hours_yesterday muze byt z spot.56k.guru fallbacku
...
if __name__ == "__main__":
    ...
    print(f"VERSION: regelleistung-xlsx-v16-yesterday", flush=True)
