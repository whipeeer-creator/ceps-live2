    ...
    def do_GET(self):
        ...
        if parsed.path == "/health":
            self._json({"status": "ok", "time": datetime.now().isoformat(),
                        "version": "regelleistung-xlsx-v13-last8h"}); return
        ...
        try:
            ...
            try:
                ...
                if r2.status_code == 200:
                    ...
                    hours_yesterday = day_data.get("hoursYesterday", [])  # nemusi byt poskytovano
                    if hours_today:
                    ...
                    # last_8h: 8 poslednich hodin (ending at current_hour, vcetne)
                    # Napr. current_hour=2 -> hodiny [yesterday-19, ..., yesterday-23, today-0, today-1, today-2]
                    if current_hour is not None and hours_today:
                        ch = current_hour
                        last_8h_list = []
                        # Posledni 8 hodin = ch-7 az ch (inclusive)
                        for offset in range(-7, 1):
                            ...
                            if target_hour >= 0:
                                ...
                                # Predchozi den
                                row = next((h for h in hours_yesterday if h.get("hour") == target_hour + 24), None)
                            if row and row.get("priceEur") is not None:
                                ...
                                # Pokud chybi (napr. yesterday neni v API), dej None
                                last_8h_list.append({
                                    "hour": target_hour % 24,
                                    "price_eur": None,
                                    "price_czk": None,
...
if __name__ == "__main__":
    ...
    print(f"VERSION: regelleistung-xlsx-v13-last8h", flush=True)
